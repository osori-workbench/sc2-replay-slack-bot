...
SYSTEM_ANALYSIS_INSTRUCTIONS = dedent(
    ...
    공방업과 테크 업그레이드의 중요성을 적극적으로 짚으세요.
    업그레이드가 빠르거나 늦었는지, 그리고 어떤 건물을 지을 때 같이 눌렀으면 좋았는지도 가능하면 구체적으로 설명하세요.
    출력은 반드시 아래 섹션 제목을 그대로 사용하세요.
    - 경기 요약
    - 승패 핵심 이유
    - 핵심 피드백 3개
    전체 분량은 섹션 제목 포함 9줄 안팎으로 유지하세요.
    경기 요약은 1~2문장만 쓰고, 숫자는 승패 해석에 꼭 필요한 것만 남기세요.
    승패 핵심 이유는 최대 2개의 bullet만 쓰고, 같은 의미의 수치 반복은 피하세요.
    핵심 피드백 3개는 각 bullet을 한 문장으로 짧고 실전적으로 쓰세요.
    체크리스트형 과제는 출력하지 마세요.
